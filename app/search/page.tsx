...
import { ExternalLink, Loader2, Search, SlidersHorizontal } from 'lucide-react';
...
function ProductCard({ item }: { item: NaverItem }) {
      ...
      <div className="mx-auto max-w-6xl px-4 py-6">
        ...
        <div className="mt-4">
          {isFetching ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
              {items.map((item) => <ProductCard key={item.id || item.link} item={item} />)}
            </div>
          ) : query ? (
